...
<template>
  <div class="flex flex-col gap-6 w-full" v-if="connection">
    <div class="flex items-center gap-3">
      ...
      <h1 class="text-2xl font-semibold text-n-slate-12">
        {{ connection?.name || t('WHATSAPP_CONNECTIONS.TITLE') }}
      </h1>
      <span
      ...
      >
        {{ t(`WHATSAPP_CONNECTIONS.STATUS.${(connection?.status || 'active').toUpperCase()}`) }}
      </span>
    </div>
